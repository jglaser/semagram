# Lessons: two architectures that did not work, and why

This file is the record of everything that came before the braided layer in the
README. It is kept because the negative results are the reason the current
design looks the way it does, and because each one cost a measurement that
somebody else would otherwise have to repeat.

Read in order, the arc is: an architecture with three exactly-verifiable
commitments (Part I), the discovery that all three are worth nothing even on
data where they are literally true (Part II), and the diagnosis that says why
(the symmetries were too cheap to be worth imposing) -- which is the hypothesis
the README then tests and confirms.
